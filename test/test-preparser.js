/*
 * By Ray Toal:
 * https://github.com/rtoal/plainscript/blob/master/test/preparser/preparser-test.js
 *
 * Preparser Tests
 *
    ...
    fs.readdirSync(fileDirectory).forEach((name) => {
      if (name.endsWith('.pls')) {
        it(`produces the correct indent/dedent markup for ${name}`, (done) => {
          fs.readFile(`${fileDirectory}/${name}`, 'utf-8', (err, input) => {
            fs.readFile(`${fileDirectory}/${name}.expected`, 'utf-8', (_err, expected) => {
